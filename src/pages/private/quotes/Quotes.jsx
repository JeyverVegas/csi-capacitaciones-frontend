...
const Quotes = () => {
    ...
    const [{ quotes, total, numberOfPages, error: quotesError, loading }, getQuotes] = useQuotes({ params: { ...filters }, options: { useCache: false } });

    const [{ error: deleteError, loading: deleteLoading }, deleteQuotes] = useAxios({ method: 'DELETE' }, { manual: true, useCache: false });
    ...
    const handleDelete = (value) => {
        deleteQuotes({ url: `/quotes/${value?.id}` }).then((data) => {
            setCustomAlert({
    ...
    const handleDeleteSelected = () => {
        deleteQuotes({ url: `/quotes/multiple`, data: { ids: selectedValues } }).then((data) => {
            setCustomAlert({
                title: '¡Operación Exitosa!',
                severity: 'success',
    ...
    return (
        <div>
            ...
            <CustomTable
                ...
                selectAll={selectAll}
                title={'Cotizaciones'}
                updatePath={"/cotizaciones/detalles"}
                entity={"quotes"}
